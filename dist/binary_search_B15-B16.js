...
// TS Code | LeetCode: 704 – Binary Search | GFG: Binary Search
function binarySearchRec(nums, target, low, high) {
    if (low > high)
        return -1;
    const mid = low + Math.floor((high - low) / 2);
    if (nums[mid] === target)
...
function searchRangeOpt(nums, target) {
...
function lowerBoundOpt(nums, target) {
    let low = 0;
...
function binarySearchUT(arr, target) {
    let low = 0;
    let high = arr.length;
    while (low < high) {
        const mid = Math.floor((low + high) / 2);
...
function searchInsertLB(arr, target) {
...
function searchPivotMethod(nums, target) {
    const n = nums.length;
    // Step 1: Find pivot
    let low = 0;
    let high = n - 1;
