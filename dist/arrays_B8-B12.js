...
function sumRangeBF(nums, left, right) {
    let sum = 0;
...
function isSortedRecursive(arr, n) {
    if (n === 1)
        return true;
    if (arr[n - 2] > arr[n - 1])
        return false;
...
function mergeSortBF(arr) {
    const temp = new Array(arr.length);
    divide(arr, temp, 0, arr.length - 1);
    return arr;
}
function divide(arr, temp, left, right) {
    if (left >= right)
        return;
...
function mergeBF(arr, temp, left, mid, right) {
