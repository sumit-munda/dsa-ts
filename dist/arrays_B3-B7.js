...
function largestElement(nums) {
    ...
    for (let i = 1; i < nums.length; i++) {
        ...
        if (nums[i] > max)
            max = nums[i]; // micro optimization / avoid function call overhead
    }
    return max;
}
...
function frequencyCountBruteForce0(nums) {
    const n = nums.length;
    for (let i = 0; i < n; i++) {
        let count = 0;
        for (let j = 0; j < n; j++) {
...
function frequencyCountBruteForce1(nums) {
    const n = nums.length;
...
function frequencyCountSort(nums) {
    nums.sort((a, b) => a - b);
    let count = 1;
    for (let i = 1; i <= nums.length; i++) {
        if (nums[i] === nums[i - 1]) {
...
function frequencyCountHashMap(nums) {
    const freq = new Map();
    for (let num of nums) {
...
function findAppearingOnceMi(nums) {
    const freq = new Map();
    for (let num of nums) {
        freq.set(num, (freq.get(num) || 0) + 1);
    }
...
// O(n)|O(n): Use a hash map to count in one pass, extra map storage
function findAppearingOnceUsingReduce(nums) {
    return nums.reduce((acc, cur) => acc ^ cur, 0);
}
// O(n)|O(1): reduce() iterates once over the array
function findAppearingOnceXOR(nums) {
    let xor = 0;
    for (let num of nums) {
        xor ^= num;
    }
...
function equalArraysMi(nums1, nums2) {
...
function maxSubarraySumBF(arr, k) {
    let n = arr.length;
    let maxSum = -Infinity;
    for (let i = 0; i <= n - k; i++) {
        let currSum = 0;
...
// TS Code | GFG: Longest Subarray with Sum K
function longestSubarraySumKSW(arr, k) {
    let left = 0;
    let sum = 0;
    let length = 0;
    for (let right = 0; right < arr.length; right++) {
...
function findMaxConsecutiveOnesSLW(nums) {
    let left = 0;
    let length = 0;
...
function isNumericBF(s) {
    for (let char of s) {
        if (isNaN(+char))
            return false; // converision overhead
    }
    return true;
}
// O(n)|O(1): Single loop
